 ...
 */
class AuthorizationException(user: User, action: Permission, resource: Any) extends Exception(s"${user.mail} does not have permission to ${action.getClass.getSimpleName} in ${resource.getClass.getSimpleName}")
