 ...
 */
class SubForumTest extends Specification with ResultMatchers {
  ...
  "Subforum" should {
    ...
    "be invalid without name" in new Ctx {
      val invalidSubforum = validSubforum.copy(name = "")
        moderator is Moderator(at = invalidSubforum)

        invalidSubforum.validate(defaultPolicy) should
